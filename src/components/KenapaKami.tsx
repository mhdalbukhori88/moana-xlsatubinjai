      ...
      <div className="container-main relative px-4 sm:px-6 lg:px-8">
        <div className="mb-8 sm:mb-12 text-center lg:mb-16">
          <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-primary/5 px-3 sm:px-4 py-1.5 sm:py-2">
            ...
            <span className="text-[10px] sm:text-xs font-semibold text-primary uppercase tracking-wider">Keunggulan</span>
          </div>
          <h2 className="mb-3 sm:mb-4 text-2xl sm:text-3xl font-bold text-primary md:text-4xl lg:text-5xl">
            Kenapa Pilih Kami?
          </h2>
          <p className="mx-auto max-w-2xl text-sm sm:text-base text-text-muted sm:text-lg">
        ...
        <div className="grid gap-4 sm:gap-6 grid-cols-2 lg:grid-cols-4">
          {features.map((feature) => (
            <div
              key={feature.title}
              className="group relative rounded-2xl sm:rounded-3xl bg-white p-4 sm:p-6 shadow-md transition-all duration-500 hover:-translate-y-2 hover:shadow-xl md:p-8"
            >
              {/* Icon */}
              <div className="mb-3 sm:mb-5 inline-flex h-10 w-10 sm:h-14 sm:w-14 items-center justify-center rounded-xl sm:rounded-2xl bg-gradient-to-br from-primary/10 to-secondary/10 text-primary transition-all duration-300 group-hover:scale-110 group-hover:from-primary group-hover:to-support group-hover:text-white group-hover:shadow-lg">
                {feature.icon}
              </div>

              <h3 className="mb-2 sm:mb-3 text-sm sm:text-lg font-bold text-text">{feature.title}</h3>
              <p className="text-xs sm:text-sm leading-relaxed text-text-muted">{feature.description}</p>

              {/* Bottom accent */}
              <div className="absolute bottom-0 left-4 right-4 sm:left-6 sm:right-6 h-1 rounded-t-full bg-gradient-to-r from-primary to-secondary opacity-0 transition-all duration-300 group-hover:opacity-100" />
            </div>
          ))}
        </div>
      </div>
